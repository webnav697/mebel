$(document).ready(function () {
	...
	if (LastSumm > 600) {
		...
		kp = k / 2 + 28;
		$(".nv_topnav>ul>li:nth-child(1n+2)>a>span").css({
			"padding": "0 " + kp + "px"
		})
	}


	$(".nv_topnav .current").next("li").find("span").css({
		"border-left": "none"
	})
	$(".nv_topnav .current").prev("li").find("span").css({
		"border-right": "none"
	})
...
function beforgarm(clk, ParentElement) {
	if (ParentElement.hasClass('open')) {
		ParentElement.find(".vc_showchild").animate({
			'opacity': 0
		}, 100)
		ParentElement.find(".vc_showchild-2").animate({
			'opacity': 1
		}, 300)
